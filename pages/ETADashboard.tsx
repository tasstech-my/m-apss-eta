...
const initialApplications: ETAApplication[] = [
    { id: 'ETA-1001', applicantName: 'John Smith', nationality: 'GBR', passportNumber: 'G12345678', submissionDate: '2023-10-28 10:15', status: 'Approved', riskScore: 10 },
    { id: 'ETA-1002', applicantName: 'Maria Garcia', nationality: 'ESP', passportNumber: 'E87654321', submissionDate: '2023-10-28 10:20', status: 'Pending Review', riskScore: 75, riskReasons: ['Fuzzy Watchlist Match'] },
    { id: 'ETA-1003', applicantName: 'Wei Chen', nationality: 'CHN', passportNumber: 'C99887766', submissionDate: '2023-10-28 10:30', status: 'Approved', riskScore: 25 },
    { id: 'ETA-1004', applicantName: 'Ahmed Al-Fayed', nationality: 'EGY', passportNumber: 'E11223344', submissionDate: '2023-10-28 11:05', status: 'Denied', riskScore: 95, riskReasons: ['Confirmed Watchlist Hit'], officerNotes: 'Denied based on intelligence report.' },
];
...
const ReviewModal: React.FC<{ app: ETAApplication; onClose: () => void; onDecide: (id: string, decision: 'Approved' | 'Denied', notes: string) => void }> = ({ app, onClose, onDecide }) => {
    const [notes, setNotes] = useState('');
    
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 transition-opacity duration-300">
            <Card title="Manual Vetting Workspace" className="w-full max-w-lg animate-scale-in">
                ...
                <div className="bg-red-50 border-l-4 border-red-500 p-3 rounded mb-4">
                    ...
                    <ul className="list-disc list-inside text-sm text-red-700">
                        {app.riskReasons?.map((reason, idx) => <li key={idx}>{reason}</li>) || <li>No specific reasons logged.</li>}
                    </ul>
                </div>
...
export const ETADashboard: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            {selectedApp && <ReviewModal app={selectedApp} onClose={() => setSelectedApp(null)} onDecide={handleDecision} />}
                    ...
                    <Card title="Live Application Feed & Vetting Engine">
                        <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                            {applications.map(app => (
                                <div key={app.id} className="flex items-center justify-between p-3 border rounded-lg bg-white shadow-sm new-alert-row">
                                    <div>
                                    ...
                                    <div className="flex items-center space-x-4">
                                        ...
                                        {app.status === 'Pending Review' && (
                                            <button onClick={() => setSelectedApp(app)} className="px-3 py-1 bg-brand-secondary text-white text-xs rounded hover:bg-brand-primary">
                                                Review
                                            </button>
                                        )}
