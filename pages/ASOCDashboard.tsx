...
const getStatusColor = (status: EquipmentStatus['status']) => {
    switch(status) {
        ...
        case 'Maintenance Required': return 'bg-amber-100 text-amber-800';
...
const getSecurityAlertSeverityPillClass = (severity: SecurityAlert['severity']) => {
    switch (severity) {
...
export const ASOCDashboard: React.FC = () => {
  ...
  const [securityAlertFilter, setSecurityAlertFilter] = useState<string>('All');
  ...
  const displayedSecurityAlerts = useMemo(() => {
    if (securityAlertFilter === 'All') {
        return alerts;
    }
    return alerts.filter(alert => alert.severity === securityAlertFilter);
  }, [alerts, securityAlertFilter]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      {/* Active Security Alerts */}
       <Card title={
        <div className="flex justify-between items-center">
            ...
            <div>
                <label htmlFor="security-alert-filter" className="text-sm font-medium text-gray-700 mr-2">Severity:</label>
                <select
                    id="security-alert-filter"
                    value={securityAlertFilter}
                    onChange={(e) => setSecurityAlertFilter(e.target.value)}
                    ...
                    aria-label="Filter security alerts by severity"
                >
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {displayedSecurityAlerts.length > 0 ? displayedSecurityAlerts.map(alert => (
              <tr key={alert.alertId}>
                <td className="px-4 py-3 whitespace-nowrap"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getSecurityAlertSeverityPillClass(alert.severity)}`}>{alert.severity}</span></td>
                ...
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{alert.status}</td>
                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 font-mono">{alert.timestamp}</td>
              </tr>
            )) : (
              <tr>
                  <td colSpan={5} className="text-center py-6 text-gray-500">
                      No active security alerts matching criteria.
                  </td>
              </tr>
            )}
          </tbody>
